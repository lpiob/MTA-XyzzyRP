...
triggerKey = "f2" -- default button to open/close scoreboard
drawOverGUI = true -- draw scoreboard over gui?
seperationSpace = 20 -- the space between top/bottom screen and scoreboard top/bottom in pixels
...
scoreboardContent = {}
firstVisibleIndex = 1
--rtBy = { ["what"] = "__NONE__", ["dir"] = -1 } -- -1 = dec, 1 = asc
sortBy = { ["what"] = "id", ["dir"] = -1 } -- -1 = dec, 1 = asc
...
function toggleScoreboard( _, state )
	...
	if state then
		if scoreboardToggled then
			showCursor(false)
		else
			if (isCursorShowing()) then return end	-- jesli kursor jest pokazywany to nie pokazujemy taba - wprowadzone bo kolidowalo z okienkiem wejscia do domu
			showCursor(true)
		end
...
--[[
						    ...
						    -- character and  getColorFromString( "#FF0000" ) },1 or {0,0,0,255},1
							local colorCodes={}--aqq
--							table.insert( colorCodes, { { 255,255,255 }, 1 } )
							local premium=getElementData(player,"premium")
							local zamaskowany=getElementData(player,"zamaskowany")
							if (not character) then
							    table.insert( colorCodes, {{100,100,100},1})
							elseif (zamaskowany) then
								table.insert( colorCodes, {{55,55,55},1})						
							elseif (premium) then
							    table.insert( colorCodes, {{255,255,155},1})						
							else
							    table.insert( colorCodes, {{255,255,255},1})						
							end
				 ...
				 if ( countPlayersInTeam(team)>0 ) then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							teamName = teamName .. " (" .. teamMemberCount .. " gracz" .. iif( teamMemberCount == "1", "", "y" ) .. ")"
							if serverInfo.allowcolorcodes then
...
--			if serverInfo.server and showServerInfo then
			...
			dxDrawImage( topX-s(10), y-25, 200, 31, "lss.png", 0,0,0, cWhite, drawOverGUI )

			dxDrawText( "http://lss-rp.pl/", topX+s(5), y+s(1), topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )

			if serverInfo.players and showServerInfo then
				local players = getElementsByType( "player" )
				local text = "Graczy: " .. tostring( #players ) .. "/" .. serverInfo.players
				...
				y = y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont )
			end
--			if (serverInfo.server or serverInfo.players) and showServerInfo then  end
...
--[[
			...
			local textLength = dxGetTextWidth( "Wciśnij F2 aby zamknąć", fontscale(rmbFont, s(0.75)), rmbFont )
			local textHeight = dxGetFontHeight( fontscale(rmbFont, s(0.75)), rmbFont )
			dxDrawText( "Wciśnij F2 aby zamknąć", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), rmbFont, "left", "top", false, false, drawOverGUI )
			...
			while ( index < firstVisibleIndex+maxPerWindow and scoreboardContent[index] ) do
				...
				elseif element and isElement( element ) and getElementType( element ) == "player" then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							if type( content ) == "table" and column.name == "name" then
								local playerName = content[1]
								local colorCodes = content[2]
								local xPos = topX+theX
								for k, v in ipairs( colorCodes ) do
...
function scoreboardResetColumns( fromServer )
	...
	if not fromServer then
		scoreboardAddColumn( "name", 300, "Imię i nazwisko" )
