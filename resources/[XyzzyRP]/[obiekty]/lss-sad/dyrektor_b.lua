...
local drzwi={}
drzwi.obiekt=createObject(1495,1298.6602783203,-925.19732666016,1440.7755126953,0,0,270)
setElementData(drzwi.obiekt,"customAction",{label="Otwórz/zamknij",resource="lss-sad",funkcja="menu_drzwi",args={}})

-- dimension
setElementDimension(drzwi.obiekt, 21)  -- bylo drzwi, ma byc drzwi.obiekt (tak jak wyzej)
-- interior
...
drzwi.otworz=function()
    ...
    drzwi.animacja=true
    moveObject(drzwi.obiekt,3000,1298.6602783203,-925.19732666016,1440.7755126953,0,0,90,"OutBounce")
...
local function pracownik(id)
    local query=string.format("SELECT rank FROM lss_character_factions WHERE faction_id=17 AND character_id=%d", id)
    local wynik=exports.DB:pobierzWyniki(query)
    if (wynik and wynik.rank and tonumber(wynik.rank)>=6 and tonumber(wynik.rank)<=6) then return true else return false end
end
...
addEvent("onSadDrzwiToggleRequest", true)
addEventHandler("onSadDrzwiToggleRequest", resourceRoot, drzwi.toggle)
