...
local obiekty={ "rolka1"  }
		-- , "statek5", "statek6", "statek7", "statek8", "statek9", "statek10", "statek11", "statek12", "statek13", "statek14", "statek15", "statek16", "statek17", "statek18"}


local sekwencja={
	{ nazwa="1", czas=3, opoznienie=0.05 },	
	{ nazwa="2", czas=3, opoznienie=0.05 },
	{ nazwa="3", czas=3, opoznienie=0.05 },
	{ nazwa="4", czas=3, opoznienie=0.05 },
	{ nazwa="5", czas=3, opoznienie=0.05 },	
	{ nazwa="6", czas=3, opoznienie=0.05 },		
...
--[[
    ...
    <object id="statek1" doublesided="false" model="10771" interior="0" dimension="0" posX="-2334.8271484375" posY="2159.91796875" posZ="5.4443907737732" rotX="0" rotY="0" rotZ="270"
		posX_bayside="..." posY_bayside="...." posZ_bayside="..." rotX_bayside="..." rotY_bayside="...." rotZ_bayside="..."
		posX_sf="..." posY_sf="..." posZ_sf="..." rotX_sf="..." rotY_sf="..." rotZ_sf="..."></object>
...
--	outputDebugString("Sekwencja " .. sekwencja[krok].nazwa)
	...
	for _,nazwa_obiektu in ipairs(obiekty) do
		...
		if (obiekt) then
			...
			local dx,dy,dz=getElementRotation(obiekt)
--			outputChatBox(string.format("Polozenie rolek: %.1f,%.1f,%.1f, obrot: %.1f,%.1f,%.1f", cx,cy,cz, dx,dy,dz))
			if (not dx) then dx=0 end
			if (not dy) then dy=0 end
			if (not dz) then dz=0 end
			...
			local rotX=getElementData(obiekt,"rotX"..postfix)
			if (not rotX) then rotX=dx end

			local rotY=getElementData(obiekt,"rotY"..postfix)
			if (not rotY) then rotY=dy end
			local rotZ=getElementData(obiekt,"rotZ"..postfix)
			if (not rotZ) then rotZ=dz end			
			moveObject(obiekt, sekwencja[krok].czas*1000, posX, posY, posZ, rotX, rotY, rotZ)
		end
	end
