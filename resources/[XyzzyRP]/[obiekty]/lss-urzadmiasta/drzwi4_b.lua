-- biuro dyrektora SM
local drzwi={}
drzwi.obiekt=createObject(1495,1472.0548095703,-1710.5618896484,1131.8264160156,0,0,180)
setElementData(drzwi.obiekt,"customAction",{label="Otwórz/zamknij",resource="lss-urzadmiasta",funkcja="menu_drzwi4",args={}})
...
drzwi.otworz=function()
    ...
    drzwi.animacja=true
    moveObject(drzwi.obiekt,3000,1472.0548095703,-1710.5618896484,1131.8264160156,0,0,-90,"OutBounce")
...
local function pracownik(id)
    local query=string.format("SELECT rank FROM lss_character_factions WHERE faction_id=4 AND character_id=%d", id)
    local wynik=exports.DB:pobierzWyniki(query)
    if (wynik and wynik.rank and tonumber(wynik.rank)>=4 and tonumber(wynik.rank)<=6) then return true else return false end
end
...
addEvent("onUM_Drzwi4ToggleRequest", true)
addEventHandler("onUM_Drzwi4ToggleRequest", resourceRoot, drzwi.toggle)
