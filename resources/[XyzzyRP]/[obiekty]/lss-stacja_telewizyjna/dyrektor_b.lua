...
local drzwi={}
drzwi.obiekt=createObject(1495,448.40377807617,-1289.0557861328,1195.3181152344,0,0,180)
setElementData(drzwi.obiekt,"customAction",{label="Otwórz/zamknij",resource="lss-stacja_telewizyjna",funkcja="menu_drzwi",args={}})

-- dimension
setElementDimension(drzwi.obiekt, 7)  -- bylo drzwi, ma byc drzwi.obiekt (tak jak wyzej)
-- interior
...
drzwi.otworz=function()
    ...
    drzwi.animacja=true
    moveObject(drzwi.obiekt,3000,448.40377807617,-1289.0557861328,1195.3181152344,0,0,90,"OutBounce")
...
local function pracownik(id)
    local query=string.format("SELECT rank FROM lss_character_factions WHERE faction_id=5 AND character_id=%d", id)
    local wynik=exports.DB:pobierzWyniki(query)
    if (wynik and wynik.rank and tonumber(wynik.rank)>=5 and tonumber(wynik.rank)<=6) then return true else return false end
end
...
addEvent("onRCDrzwiToggleRequest", true)
addEventHandler("onRCDrzwiToggleRequest", resourceRoot, drzwi.toggle)
