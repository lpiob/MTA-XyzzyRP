...
function auth_fetchPlayerCharacters(player,userid)
    -- onCharacterSetReceived
--    local query=string.format("SELECT id,imie,nazwisko,skin,tytul,lastpos,hp,ar,money,newplayer,fingerprint FROM lss_characters WHERE userid=%d;", userid)
    ...
    local characters=exports.DB:pobierzTabeleWynikow(query)
